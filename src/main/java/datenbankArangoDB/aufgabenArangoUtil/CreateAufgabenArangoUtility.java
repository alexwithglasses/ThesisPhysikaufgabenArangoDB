...
public class CreateAufgabenArangoUtility {
    ...
    public static String ersetzeUmlaute(String original) {
        String[][] UMLAUT_UNICODE = { { new String("Ä"), "\u00c4" }, { new String("Ü"), "\u00dc" }, { new String("Ö"), "\u00d6" }, { new String("ä"), "\u00e4" }, { new String("ü"), "\u00fc" }, { new String("ö"), "\u00f6" }, { new String("ß"), "\u00df" } };

        String unicode = original;

        for (int i = 0; i < UMLAUT_UNICODE.length; i++) {
            unicode = unicode.replace(UMLAUT_UNICODE[i][0], UMLAUT_UNICODE[i][1]);
        }
